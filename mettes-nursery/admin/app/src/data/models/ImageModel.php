<?php

/*
 * To change this license header, choose License Headers in Project Properties. 
 * To change this template file, choose Tools | Templates 
 * and open the template in the editor. 
 */

/**
 * Description of ImageModel 
 * 
 * @author ThomasElvin 
 */
class ImageModel {

    private $_db;

    public function __construct() {
        $this->_db = DB::getInstance();
    }

    //Get all users 
    public function getImages() {
        $this->_db->get(array('*'), IMAGES_TABLE, null);
        return $this->_db->results();
    }

    public function getImagesToProduct($ID) {
        $this->_db->get(array('*'), IMAGES_TABLE, array(IMAGE_FK_ID, '=', $ID));
        return $this->_db->results();
    }

    public function getImage($ID) {
        $this->_db->get(array('*'), IMAGES_TABLE, array(IMAGE_ID, '=', $ID));
        return $this->_db->results();
    }

    public function delete($ID) {
        return $this->_db->delete(IMAGES_TABLE, array(IMAGE_ID, '=', $ID));
    }

    public function deleteProductImages($ID) {
        return $this->_db->delete(IMAGES_TABLE, array(IMAGE_FK_ID, '=', $ID));
    }

    public function addToImages($file, $original, $ID = null) {
        $DB_ID = $this->_db->lastInsertID();
        
        if (empty($DB_ID)) {
            $Upload_ID = $ID;
        } else {
            $Upload_ID = $DB_ID;
        }
        
        $data = array(
            IMAGE_FK_ID => $Upload_ID,
            IMAGE_ORIGINAL => $original,
            IMAGE_URL => $file,
            IMAGE_DATE => time()
        );
        return $this->_db->insert(IMAGES_TABLE, $data);
    }

}
