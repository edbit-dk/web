...
class ControllerPaymentQuickPayPaii extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			$this->model_setting_setting->editSetting('quickpay_paii', $this->request->post);				
		...
		$data['action'] = $this->url->link('payment/quickpay_paii', 'token=' . $this->session->data['token'], 'SSL');
		
		$data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');
		
		
		
		$fields = array('quickpay_paii_status', 
						'quickpay_paii_protocol', 
						'quickpay_paii_merchant', 
						'quickpay_paii_language', 
						'quickpay_paii_secret', 
						'quickpay_paii_msgtype', 
						'quickpay_paii_order_status_completed', 
		...
		if (!isset($data['quickpay_paii_status'])) $data['quickpay_paii_status'] = "0";
		if (!isset($data['quickpay_paii_protocol'])) $data['quickpay_paii_protocol'] = "7";
		if (!isset($data['quickpay_paii_language'])) $data['quickpay_paii_language'] = "da";
		if (!isset($data['quickpay_paii_msgtype'])) $data['quickpay_paii_msgtype'] = "authorize";
		if (!isset($data['quickpay_paii_order_status_completed'])) $data['quickpay_order_status_completed'] = "5";
		if (!isset($data['quickpay_paii_autocapture'])) $data['quickpay_paii_autocapture'] = "0";
		if (!isset($data['quickpay_paii_splitpayment'])) $data['quickpay_paii_splitpayment'] = "1";
		...
		$this->response->setOutput($this->load->view('payment/quickpay_paii.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/quickpay_paii')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		$errors = array('protocol', 'msgtype', 'merchant', 'md5check');
		foreach ($errors as $error) {
			if (!$this->request->post['quickpay_paii_' . $error]) {
