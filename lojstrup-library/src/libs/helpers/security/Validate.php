...
class Validate implements SingletonInterface
{
            ...
            function load($params = null)
    {
        ...
        {
            self::$_instance = new Validate($params);
        }
            ...
            foreach ($rules as $rule => $rule_value)
            {
                ...
                else if (!empty($value))
                {
                    switch ($rule)
                    {
                        ...
                        case 'notTaken':
                            $check = $this->_storage->select(array($item), $rule_value, array($item, '=', $value), 'LIMIT 1');
                            if ($check->count())
                            {
                ...
                if (!empty($value))
                {
                    switch ($rule)
                    {
                        case 'ext':
                            ...
                            {
                                $this->addError("<p style='color: red;' class='error'>'.{$file_extension}' extension not allowed! Please use " . implode(', ', $rule_value) . ".</p>");
                            }
                            break;
                    }
                }
